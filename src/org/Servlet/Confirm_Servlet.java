...
public class Confirm_Servlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String hotelname=d.gethotelname(da.getMemail(),da.getLocation());
			Bookings b=new Bookings(id,title,date,name,email,status,hotelname,da.getSemail(),da.getMemail(),da.getFoodcast(),da.getTranspotcost(),da.getLodgecost(),tot,du,no);
			d.addbookings(b);
			int n=d.getno(da.getMemail(),hotelname);
			d.updateno(n+1,hotelname);

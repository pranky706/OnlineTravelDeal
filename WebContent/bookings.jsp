...
<%
	...
	{
%>
<tr><td><%= m.getDealid() %></td><td><%= m.getTitle() %></td><td><%= m.getBdate() %></td><td><%= m.getCname() %></td><td><%= m.getCemail() %></td><td><%= m.getStatus() %></td><td><a href="Change_Status.jsp?email=<%= m.getCemail()%>&name=<%= m.getCname()%>&bdate=<%= m.getBdate()%>">To Change status click here</a></td></tr>
<% } %>
</table>
<% }
	else
...
<a align="center" href="Fetch_Merchant_Details">Click here to go to <%= Name %>'s home page</a>
<%} %>
<div id="footer">
